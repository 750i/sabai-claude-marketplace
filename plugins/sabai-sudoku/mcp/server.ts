...
interface SudokuGame {
  id: string;
  puzzle: number[][];      // The puzzle with empty cells (0 = empty)
  solution: number[][];    // The complete solution
  current: number[][];     // Current state of the board
...
const games = new Map<string, SudokuGame>();
...
function createPuzzle(solution: number[][], difficulty: Difficulty): number[][] {
  ...
  const removeCount: Record<Difficulty, number> = {
    easy: 35,
    medium: 45,
    hard: 52,
    expert: 58
...
// Analyze the board and find the best hint with explanation
...
function analyzeForHint(game: SudokuGame): HintAnalysis | null {
  ...
  for (let row = 0; row < 9; row++) {
    for (let num = 1; num <= 9; num++) {
      ...
      let found = false;
      for (let c = 0; c < 9; c++) {
        if (board[row][c] === num) { found = true; break; }
      }
      if (found) continue;
  ...
  for (let col = 0; col < 9; col++) {
    for (let num = 1; num <= 9; num++) {
      ...
      let found = false;
      for (let r = 0; r < 9; r++) {
        if (board[r][col] === num) { found = true; break; }
      }
      if (found) continue;
...
export function createServer(): McpServer {
  ...
  registerAppTool(server,
    ...
    async (args): Promise<CallToolResult> => {
      ...
      const errors: { row: number; col: number; current: number; expected: number }[] = [];
